...
describe("Tag Extraction Logic", () => {
  ...
  test("should extract multiple tags", () => {
    expect(
      extractTags("This message has #multiple #tags"),
    ).toEqual(["#multiple", "#tags"]);
  });
  ...
  test("should extract tag in the middle", () => {
    expect(
      extractTags("Message with a #middle_tag here"),
    ).toEqual(["#middle_tag"]);
  });

  test("should extract tag at the end", () => {
    expect(extractTags("Message ends with #end")).toEqual([
      "#end",
    ]);
  });
  ...
  test("should extract multiple tags appearing consecutively", () => {
    ...
    expect(extractTags("Tags #one #two #three")).toEqual([
      "#one",
      "#two",
      "#three",
    ]);
    expect(extractTags("Tags #one#two #three")).toEqual([
      ...
      "#two",
      "#three",
    ]); // Correction: This should be ['#one', '#two', '#three'] if #two is a valid tag start, but it's not. It will be ['#one', '#three']
  ...
  test("should extract tags with numbers", () => {
    expect(extractTags("Tag with #number123")).toEqual([
      "#number123",
    ]);
  });

  test("should extract tags with underscores", () => {
    expect(extractTags("Tag with #under_score")).toEqual([
      "#under_score",
    ]);
  });
  ...
  test("should return an empty array for messages with special characters but no valid tags", () => {
    ...
    expect(extractTags("Message with #tag-with-hyphen")).toEqual([
      "#tag",
    ]); // Only #tag will be matched
  });
  ...
  test("should correctly extract valid tags when mixed with invalid ones", () => {
    expect(
      extractTags(
        "A #valid tag and an #invalid-tag with hyphen, also #another_valid.",
      ),
    ).toEqual(["#valid", "#invalid", "#another_valid"]);
  });
  ...
  test("should handle multiple hashtags correctly", () => {
    ...
    expect(extractTags("#tag#another")).toEqual([
      "#tag",
      "#another",
    ]);
  });
