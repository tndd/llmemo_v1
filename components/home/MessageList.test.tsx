...
describe("Message Filtering Logic (as in MessageList)", () => {
  ...
  test("should correctly filter messages when some have no tags defined or empty tags array", () => {
    const filteredByTest = filterMessages(
      sampleMessages,
      "#test",
    );
    ...
    const filteredByNews = filterMessages(
      sampleMessages,
      "#news",
    );
  ...
  test("should handle messages with undefined tags property gracefully", () => {
    ...
    const messagesWithUndefined: Message[] = [
      ...
      },
      {
        ...
        avatar: "",
        time: "",
        text: "msg with empty tags",
        tags: [],
      },
    ];
    const filtered = filterMessages(
      messagesWithUndefined,
      "#tag1",
    );
    expect(filtered.length).toBe(1);
    expect(filtered[0].id).toBe(1);

    const filteredByNonExistent = filterMessages(
      messagesWithUndefined,
      "#tag2",
    );
