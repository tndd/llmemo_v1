...
jest.mock('./LibrarySidebar', () => {
  ...
  return jest.fn(({ onSelectTag, selectedTag, allTags }) => (
    <div data-testid="mock-library-sidebar">
      ...
      <button data-testid="mock-tag-button" onClick={() => onSelectTag('#test')}>
        #test
      </button>
      <div data-testid="selected-tag-in-sidebar">{selectedTag}</div>
      <div data-testid="all-tags-in-sidebar">{JSON.stringify(Array.from(allTags))}</div>
    </div>
  ));
});
...
jest.mock('../home/MessageList', () => {
  ...
  return jest.fn(({ messages, selectedTag }) => (
    <div data-testid="mock-message-list">
      ...
      <span data-testid="selected-tag-in-messagelist">{selectedTag}</span>
...
const TestLibraryViewLogic: React.FC<TestLibraryViewProps> = ({ messages, allTags }) => {
  ...
  return (
    <div>
      ...
      <div className="content-area">
        ...
        <MockMessageList messages={filteredMessages} selectedTag={null} />
      </div>
    </div>
  );
};
...
const sampleMessages: Message[] = [
  { id: 1, user: 'A', avatar: '', time: '', text: 'Message with #test tag', tags: ['#test', '#general'] },
  { id: 2, user: 'B', avatar: '', time: '', text: 'Message with #general tag', tags: ['#general'] },
  { id: 3, user: 'C', avatar: '', time: '', text: 'Message with no tags', tags: [] },
  { id: 4, user: 'D', avatar: '', time: '', text: 'Another #test message', tags: ['#test'] },
];
...
describe('LibraryView Logic (Conceptual)', () => {
  ...
  test('selecting a tag filters messages in MessageList', () => {
    ...
    const { filteredMessagesAfterClick, selectedTagAfterClick } = (() => {
        ...
        const filteredMessages = selectedTag
            ? sampleMessages.filter(msg => msg.tags && msg.tags.includes(selectedTag))
            : sampleMessages;
        return { filteredMessagesAfterClick: filteredMessages, selectedTagAfterClick: selectedTag };
    })();
    
    expect(selectedTagAfterClick).toBe('#test');
    expect(filteredMessagesAfterClick.length).toBe(2); // msg1 and msg4 have #test
    expect(filteredMessagesAfterClick.every(msg => msg.tags?.includes('#test'))).toBe(true);
  ...
  test('clicking the same tag again clears the filter', () => {
    ...
    const { filteredMessagesAfterSecondClick, selectedTagAfterSecondClick } = (() => {
        let selectedTag: string | null = null;
        const handleSelectTag = (tag: string) => {
            selectedTag = selectedTag === tag ? null : tag;
        };
        ...
        const filteredMessages = selectedTag
            ? sampleMessages.filter(msg => msg.tags && msg.tags.includes(selectedTag))
            : sampleMessages;
        return { filteredMessagesAfterSecondClick: filteredMessages, selectedTagAfterSecondClick: selectedTag };
    })();

    expect(selectedTagAfterSecondClick).toBe(null);
    expect(filteredMessagesAfterSecondClick.length).toBe(sampleMessages.length);
  });
});
